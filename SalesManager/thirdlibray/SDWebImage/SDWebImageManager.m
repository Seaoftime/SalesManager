...
{
    SDWIRetain(downloader);
    int index = [downloaders indexOfObject:downloader];
    id delegate = [downloadDelegates objectAtIndex:index];
    if([delegate respondsToSelector:@selector(webImageManager:didUpdateWithProgress:forURL:)])
    {
        [delegate webImageManager:self didUpdateWithProgress:progress forURL:downloader.url];
    }
    SDWIRelease(downloader);
}
