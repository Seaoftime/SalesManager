...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    if (expectedSize > 0)
    {
        if([delegate respondsToSelector:@selector(imageDownloader:didUpdateProgress:)])
        {
            CGFloat progress = (imageData.length*1.0)/expectedSize;
//            NSLog(@"Image Downloader Progress:%.1f%%",progress*100);
            [delegate imageDownloader:self didUpdateProgress:progress];
        }
        
        if (self.progressive && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)])
        {
