...
#define kTextColor [UIColor colorWithRed:90/255.0 green:90/255.0 blue:90/255.0 alpha:1.0]
...
//        self.backgroundColor = [UIColor clearColor];
        ...
        [_stateLabel setFont:[UIFont fontWithName:@"Helvetica-Bold" size:13]];

        if (isNetWork)
        _stateLabel.text = NSLocalizedString(@"下拉刷新", @"");
        else _stateLabel.text = NSLocalizedString(@"无网络连接", @"");
...
//        _dateLabel.text = NSLocalizedString(@"最后更新", @"");
        [_dateLabel setFont:[UIFont fontWithName:@"Helvetica-Bold" size:13]];
        ...
        _arrow.contents = (id)[UIImage imageWithCGImage:[UIImage imageNamed:@"blueArrow@2x.png"].CGImage scale:1 orientation:UIImageOrientationDown].CGImage;
...
//        arrowFrame = CGRectMake(4*x+60, y, kPRArrowWidth, kPRArrowHeight);
//        arrowFrame = CGRectMake(4*x+60 + 55, y, kPRArrowWidth, kPRArrowHeight);

        if (kDeviceWidth > 500.000000) {
            arrowFrame = CGRectMake(4*x+60 + 55 + 165, y, kPRArrowWidth, kPRArrowHeight);
        }else if (kDeviceWidth < 370.000000) {
            
            arrowFrame = CGRectMake(4*x+60 + 15, y, kPRArrowWidth, kPRArrowHeight);
        }else
            arrowFrame = CGRectMake(4*x+60 + 45, y, kPRArrowWidth, kPRArrowHeight);
        ...
        y = margin;
        stateFrame = CGRectMake(0, y + 10, size.width, kPRLabelHeight );
        
        y = y + kPRLabelHeight;
//        dateFrame = CGRectMake(0, y, size.width, kPRLabelHeight);
        dateFrame = CGRectMake(0, y, size.width, 0);
...
//        arrowFrame = CGRectMake(4*x+60 + 55, y, kPRArrowWidth, kPRArrowHeight);
        ...
        if (kDeviceWidth > 500.000000) {
            ...
            arrowFrame = CGRectMake(4*x+60 - 120, y, kPRArrowWidth, kPRArrowHeight);
...
//                    _stateLabel.text = NSLocalizedString(@"释放刷新", @"");
                    _stateLabel.text = NSLocalizedString(@"松开刷新", @"");
                    //
                    ...
                    _stateLabel.text = NSLocalizedString(@"无网络连接", @"");
            ...
            if (!self.isAtTop) {    //footer
                _arrow.hidden = YES;
                _stateLabel.text = NSLocalizedString(@"已经全部加载完毕", @"");
            }
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        rect = CGRectMake(0,  0 , frame.size.width, frame.size.height - 100);
...
//        rect = CGRectMake(0, self.frame.size.height, self.frame.size.width, self.frame.size.height);
        rect = CGRectMake(0,  0 , self.frame.size.width, self.frame.size.height - 100);
        _footerView = [[LoadingView alloc] initWithFrame:rect atTop:NO];
        _footerView.atTop = NO;
        [self addSubview:_footerView];
...
//    if (offset.y < -kPROffsetY) {
        ...
        [UIView animateWithDuration:kPRAnimationDuration + 0.1 animations:^{
...
//        [UIView animateWithDuration:kPRAnimationDuration  + 1 animations:^{
...
- (void)tableViewDidFinishedLoadingWithMessage:(NSString *)msg{
    ...
    if (_headerView.loading) {
        ...
        [UIView animateWithDuration:kPRAnimationDuration*2.5 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            self.contentInset = UIEdgeInsetsMake(0, 0, 0, 0);
        } completion:^(BOOL bl){
    ...
    //    if (_footerView.state == kPRStateLoading) {
        ...
        [UIView animateWithDuration:kPRAnimationDuration*2.5 animations:^{
            //self.contentInset = UIEdgeInsetsMake(0, 0, 0, 0);
        } completion:^(BOOL bl){
            if (msg != nil && ![msg isEqualToString:@""]) {
                [self flashMessage:msg];
